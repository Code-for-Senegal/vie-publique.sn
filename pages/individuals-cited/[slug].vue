...
<template>
  <h1 v-if="individualCited != null" class="sr-only">
    {{ individualCited.nom }}
  </h1>

  <div class="p-0">
    <UBreadcrumb
      v-if="individualCited != null"
      divider=">"
      :links="links"
      class="mb-2 px-2.5"
    />
    ...
    <UCard v-if="individualCited != null">
      <template #header>
        <div class="">
          <h1 class="text-2xl font-semibold">{{ individualCited.nom }}</h1>
          <p class="text-sm text-gray-500">{{ individualCited.fonction }}</p>
        </div>
      </template>

      <img
        :src="individualCited.photo"
        loading="lazy"
        fetchpriority="high"
        alt="Photo de profil"
      ...
      <div
        v-for="(factItem, i) in individualCited.facts_denounced"
        v-if="individualCited.facts_denounced != null"
        :key="i"
        class="mb-1 text-sm"
      >
        ...
        <ul class="list-disc">
          <li
            v-for="(infractionsItem, i) in factItem.infractions"
            :key="i"
            class="ml-5 mt-1"
          >
        ...
        <p v-if="factItem.intro != null" class="my-2 font-semibold">
          {{ factItem.outro }}
        </p>
      </div>
    </UCard>

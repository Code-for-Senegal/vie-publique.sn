...
const seoDescription = "Fonds d'appui et de développement de la presse (FADP), classement des titres de presse aidés en 2023 au Sénégal";
...
const filteredAndSortedAides = computed(() => {
    let filtered = data.value.aides?.filter((aide: AidePresse) =>
        aide.recipient_name.toLowerCase().includes(searchQuery.value.toLowerCase()) &&
...
const rowsfilteredAides = computed(() => {
    return filteredAndSortedAides.value.slice((page.value - 1) * pageCount, page.value * pageCount);
});
...
<template>
    ...
    <div class="flex flex-col items-center px-4">
        ...
        <div class="text-center">
            <p v-if="rowsfilteredAides.length > 1" class="text-sm mb-2 text-gray-500">
        ...
        <div class="w-full max-w-4xl">
            <UInput class="input w-full mb-3 custom-shadow" size="lg" icon="i-heroicons-magnifying-glass"
                placeholder="Rechercher un media..." v-model="searchQuery">
            </UInput>
            ...
            <div class="fle xflex-row text-center mb-4 text-sm">
                <span class="mr-2 text-gray-600">Trier par :</span>
                <a href="#" @click.prevent="sortType = 'name'" :class="{ ' text-black underline': sortType === 'name' }"
                    class="text-gray-500">Nom</a> |
                <a href="#" @click.prevent="sortType = 'amount'"
                    :class="{ 'text-black underline': sortType === 'amount' }" class="text-gray-500">
                    Montant
                </a>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                <UCard v-for="press in rowsfilteredAides" :key="press.recipient_name"
                    class="cursor-pointer custom-shadow">
                    <div class="flex flex-row gap-2 ">
                        <div class="flex-shrink-0 w-16 h-16">
                            <NuxtImg :src="press.photo || '/default-image-2.gif'" alt="Photo ministre" sizes="40px"
                                class="object-cover rounded-full w-full h-full" placeholder />

                        </div>
                        <div class="flex-grow">
                            <h2 class="text-sm font-semibold">{{ press.recipient_name }}</h2>
                            <p class="">
                                {{ $formatAmount(press.amount) }} FCFA
                            </p>
            ...
            <div :class="{ 'hidden': rowsfilteredAides < pageCount }"
                class="flex justify-end px-3 py-3.5 border-t border-gray-200 dark:border-gray-700">
                <UPagination size="md" v-model="page" :page-count="pageCount" :total="filteredAndSortedAides.length" />
            </div>
        </div>

...
const filteredRapports = computed(() => rapports.value.filter((rapport: any) => {
  return (
...
onMounted(async () => {
  rapports.value = data.value.sort((a, b) => new Date(b.date_publication).getTime() - new Date(a.date_publication).getTime());
});

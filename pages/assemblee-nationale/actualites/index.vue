...
const title = "Actualités de l'Assemblée nationale du Sénégal | Vie-Publique.sn";
const description = "Suivez toutes les actualités de l'Assemblée nationale du Sénégal. Débats parlementaires, votes, commissions et activités des députés en temps réel.";
...
const newsCollectionSchema = {
  ...
  "mainEntity": {
    ...
    "description": "Liste des dernières actualités de l'Assemblée nationale du Sénégal",
  },
};
...
useHead({
  ...
  meta: [
    ...
    { name: "news_keywords", content: "Assemblée nationale, Sénégal, politique, débats, votes, députés" },
  ],
  script: [
    {
...
<template>
  <div class="container mx-auto px-4 py-4" itemscope itemtype="https://schema.org/CollectionPage">
    ...
    <div class="mx-auto max-w-7xl">
      <h1 class="mb-8 text-center text-3xl font-bold text-gray-900 dark:text-white" itemprop="headline">
      ...
      <div v-if="loading" class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="n in 6" :key="n" class="animate-pulse">
          ...
          <div class="mt-4 h-4 w-3/4 rounded bg-gray-200 dark:bg-gray-700"></div>
          <div class="mt-2 h-3 w-1/4 rounded bg-gray-200 dark:bg-gray-700"></div>
        </div>
      </div>
      ...
      <!-- Content -->
      <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3" itemscope
        itemtype="https://schema.org/ItemList">
        <meta itemprop="numberOfItems" :content="news.length">

        <article v-for="(article, index) in news" :key="article.id" itemscope itemtype="https://schema.org/NewsArticle"
          ...
          <NuxtLink :to="`/assemblee-nationale/actualites/${article.id}/${article.slug}`" class="flex h-full flex-col"
            itemprop="url">
            ...
            <div class="flex flex-1 flex-col p-4">
              ...
              <div v-else class="mt-3 text-sm text-gray-500 dark:text-gray-400">
                <span itemprop="keywords" class="rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-600 dark:bg-gray-700 dark:text-gray-300">assemblée nationale</span>
              </div>
      ...
      <!-- Empty state -->
      <div v-if="!loading && !error && news.length === 0" class="py-12 text-center text-gray-500 dark:text-gray-400">

export const useSearchEnhanced = () => {
  ...
  const selectedTypes = ref<string[]>((route.query.types as string)?.split(',').filter(Boolean) || []);
    ...
    try {
      ...
      if (data.value) {
        ...
        searchResults.value = (data.value.data || []).map((result: any) => ({
          ...result,
          highlightedTitle: highlightText(result.document?.title || '', searchQuery.value),
          highlightedContent: result.highlights?.content_html 
            ? highlightText(result.highlights.content_html, searchQuery.value)
            : highlightText(result.document?.content_html?.substring(0, 200) || '', searchQuery.value)
        }));
  ...
  watch(selectedTypes, (newTypes, oldTypes) => {
    console.log('Filtres changés:', { newTypes, oldTypes });
    currentPage.value = 1;
    performSearch();
  }, { deep: true });
  ...
  const toggleType = (type: string) => {
    console.log('toggleType appelé avec:', type);
    console.log('selectedTypes avant:', [...selectedTypes.value]);
    ...
    console.log('selectedTypes après:', [...selectedTypes.value]);
  };

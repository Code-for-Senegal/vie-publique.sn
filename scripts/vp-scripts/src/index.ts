...
const FOOTER_TEXT = 'Vie-publique.sn - Ne pas utiliser à des fins commerciales';
...
async function processDocument(pdfBytes: Buffer, originalFileName: string): Promise<Uint8Array> {
  ...
  } else if (LOGO_PATH.toLowerCase().endsWith('.jpg') || LOGO_PATH.toLowerCase().endsWith('.jpeg')) {
    logoImage = await pdfDoc.embedJpg(logoImageBytes);
  } else {
    console.warn('Format de logo non supporté. Seuls PNG et JPEG sont pris en charge.');
  }
  ...
  for (const page of pages) {
    ...
    // Premier filigrane (diagonal au centre)
    const fontSize = 50;
    const textWidth = helveticaFont.widthOfTextAtSize(WATERMARK_TEXT, fontSize);
    ...
    // Deuxième filigrane (en bas de page, décalé à gauche)
    const footerFontSize = 10;
    const footerTextWidth = helveticaFont.widthOfTextAtSize(FOOTER_TEXT, footerFontSize);
    
    // Dessiner le texte en bas de page, décalé à gauche
    page.drawText(FOOTER_TEXT, {
      x: 20, // Décalé à gauche avec une marge de 20 points
      y: 20, // Position en bas de page
      size: footerFontSize,
      font: helveticaFont,
      color: rgb(0.5, 0.5, 0.5), // Gris moyen
